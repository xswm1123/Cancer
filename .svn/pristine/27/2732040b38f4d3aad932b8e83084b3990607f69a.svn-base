...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self performSelector:@selector(indexLoadData) withObject:nil afterDelay:0.5];
}
...
-(void)indexLoadData{
    ...
    NSString *jsonString = [UtilTool sendUrlRequestByCache:url paramValue:nil method:HTTPRequestMethodPost isReload:NO status:&status];
    
    SBJsonParser *parser = [[SBJsonParser alloc] init];
    
    if([status intValue]==3){
        [NSThread detachNewThreadSelector:@selector(getAttenceData) toTarget:self withObject:nil];
    }else{
        if(jsonString== nil){
