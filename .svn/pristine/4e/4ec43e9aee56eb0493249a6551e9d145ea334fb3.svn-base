//
//  CustomUILabel.h
//  cplus
//
//  Created by hsyouyou on 13-5-7.
//  Copyright (c) 2013年 parsec. All rights reserved.
//

#import <UIKit/UIKit.h>
#define AppFontName @"MicrosoftYaHei"
@interface CustomUILabel : UILabel

-(void)setLabelText:(NSString *)text fontSize:(CGFloat)fontSize;
-(void)setLabelText:(NSString *)text maxHeight:(CGFloat)height fontSize:(CGFloat)fontSize;

@property (nonatomic, strong) IBOutlet NSLayoutConstraint *heightLayout;
@property (nonatomic, strong) IBOutlet NSLayoutConstraint *widthLayout;
@end
